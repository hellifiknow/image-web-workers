...
(function () {
    ...
    source.onload = function () {
        ...
        if (!canvas.getContext) {
            log.innerHTML = "Canvas not supported. Please install an HTML5 compatible browser.";
            return;
        }
        ...
        var onWorkEnded = function (e) {
            ...
            if (finished == workersCount) {
                var diff = new Date() - start;
                log.innerHTML = "Process done in " + diff + " ms using " + workersCount + " workers";
            }
        };
...
/*             worker.postMessage({ data: canvasData, index: index, length: segmentLength }); */
            ...
            var objectData = { data: canvasData, index: index, length: segmentLength };
            worker.postMessage = worker.webkitPostMessage || worker.postMessage;
            worker.postMessage(objectData, [objectData.data.data.buffer]);
            if(objectData.data.data.byteLength)
            {
	            console.log("transferrable objects not supported");
            }
        }
    };

    source.src = "sample.png";
})();
